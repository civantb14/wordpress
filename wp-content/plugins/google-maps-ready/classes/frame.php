...
class frameGmp {
	...
	private function _registerU() {
		...
		if(!defined('S_YOUR_SECRET_HASH_'. GMP_CODE))
			define('S_YOUR_SECRET_HASH_'. GMP_CODE, '$#%Ffsdfsdfjkdsf$#%$#sdSadas7987f_)(*&%$#s$dfs');
	}
	public function preU($checkedData) {
		if(class_exists('wpUpdaterGmp')) {
			return wpUpdaterGmp::getInstance( GMP_PLUG_NAME, GMP_MAIN_FILE, GMP_CODE )->checkForPluginUpdate($checkedData);
		}
		return $checkedData;
	}
	public function plA($def, $action, $args) {
		if(class_exists('wpUpdaterGmp')) {
			return wpUpdaterGmp::getInstance( GMP_PLUG_NAME, GMP_MAIN_FILE, GMP_CODE )->myPluginApiCall($def, $action, $args);
		}
		return $def;
	}
}
