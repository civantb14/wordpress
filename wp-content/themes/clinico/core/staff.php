...
function cws_portfolio_register_taxonomies() {
	...
	if (0 == count(get_terms('cws-staff-dept', 'hide_empty=0')) ) {
		register_taxonomy('type', 'staff', array('hierarchical' => true, 'label' => 'Item Type'));
		$_categories = get_categories('taxonomy=type&title_li=');
		foreach ($_categories as $_cat) {
			if (!term_exists($_cat->name, 'cws-staff-dept'))
				wp_insert_term($_cat->name, 'cws-staff-dept');
		}
		$staff = new WP_Query(array('post_type' => 'staff', 'posts_per_page' => '-1'));
		while ($staff->have_posts()) :
			...
			wp_set_post_terms($post_id, $terms, 'cws-staff-dept');
		endwhile;
		wp_reset_query();
		register_taxonomy('type', array());
	}

	if (0 == count(get_terms('cws-staff-procedures', 'hide_empty=0')) ) {
		register_taxonomy('type', 'staff', array('hierarchical' => true, 'label' => 'Item Type'));
		$_categories = get_categories('taxonomy=type&title_li=');
		foreach ($_categories as $_cat) {
			if (!term_exists($_cat->name, 'cws-staff-procedures'))
				wp_insert_term($_cat->name, 'cws-staff-procedures');
		}
		$staff = new WP_Query(array('post_type' => 'staff', 'posts_per_page' => '-1'));
		while ($staff->have_posts()) :
			$staff->the_post();
			...
			wp_set_post_terms($post_id, $terms, 'cws-staff-procedures');
		endwhile;
		wp_reset_query();
		register_taxonomy('type', array());
	}

	if (0 == count(get_terms('cws-portfolio-type', 'hide_empty=0')) ) {
		register_taxonomy('type', 'portfolio', array('hierarchical' => true, 'label' => 'Item Type'));
		$_categories = get_categories('taxonomy=type&title_li=');
		foreach ($_categories as $_cat) {
			if (!term_exists($_cat->name, 'cws-portfolio-type'))
				wp_insert_term($_cat->name, 'cws-portfolio-type');
		}
		...
		while ($portfolio->have_posts()) : $portfolio->the_post();
			...
			wp_set_post_terms($post_id, $terms, 'cws-portfolio-type');
		endwhile;
		wp_reset_query();
		register_taxonomy('type', array());
	}
